...
[AUDIT-MULTITENANT.md] dan [MODEL-BISNIS.md] di repo itu.
...
Konsekuensinya, "sistem multi-tenant" terbagi jadi dua bagian yang tidak
saling mencampuri:

```
┌────────────────────────── laptop Rohan ───────────────────────────┐
│                                                                    │
│  otin-carwash/  (APLIKASI — satu kode untuk semua cucian)          │
│      Laravel 12, logika bisnis, migrasi, seeder                    │
│                 │                                                  │
│                 │ dibaca pada commit tertentu                      │
│                 ▼                                                  │
│  carwash-saas/  (ARMADA — repo ini)                                │
│      armada.json      setelan bersama: domain utama, WHM, prefix   │
...
### Kenapa aplikasinya tidak disalin ke repo ini

Menyalin `otin-carwash` ke sini berarti dua kode yang harus diperbaiki
bersamaan, padahal audit sudah membuktikan tidak ada satu pun logika bisnis
yang perlu diubah untuk cucian kedua. OTIN diperlakukan sebagai **tenant
pertama**, bukan sebagai "versi custom". Perubahan Tahap 1 (nama bisnis,
zona waktu) tetap dikerjakan di `otin-carwash` dan langsung dinikmati OTIN.

Kalau nanti nama repo `otin-carwash` terasa salah untuk produk, cukup
di-rename — riwayat commit tidak berubah, dan `armada.json` tinggal
menunjuk ke path baru.
...
**Yang harus dikerjakan di `otin-carwash` agar kontrak ini berlaku**
...
## Daftar tenant (`tenant/<slug>.json`)
...
Password ada di `rahasia/<slug>.env` (di-gitignore); token WHM di variabel
...
### 1. Cucian baru — `bin/tenant-baru.ps1`
...
3. Tulis `rahasia/budi.env` dari `templat/env.tenant`.
...
### 2. Rilis — `bin/rilis.ps1` *(berikutnya)*

Membangun **satu paket untuk semua cucian** dari `otin-carwash` pada satu commit:

- `rilis/<commit>/app.zip` — seluruh aplikasi termasuk `vendor/`, tanpa `.env`,
  `docs/`, `deploy/`, `tests/`, `android-app/`. Aturan pengecualian disalin dari
...
### 3. Update — `bin/update-semua.ps1` *(berikutnya)*
...
### 4. Backup di luar hosting — `bin/tarik-backup.ps1` *(berikutnya)*
...
1. **Sekarang, di `otin-carwash`:** `business_name` di settings + `APP_TIMEZONE`.
   OTIN ikut menikmati; tidak ada kerja terbuang.
2. **Sekarang, di repo ini:** daftar tenant + `tenant-baru.ps1` (sudah ada).
...
[AUDIT-MULTITENANT.md]: ../../otin-carwash/docs/AUDIT-MULTITENANT.md
[MODEL-BISNIS.md]: ../../otin-carwash/docs/MODEL-BISNIS.md
